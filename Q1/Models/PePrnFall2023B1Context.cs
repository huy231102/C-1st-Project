...
public partial class PePrnFall2023B1Context : DbContext
{
    ...
    private String[] GetDefaultMember()
    {
        using (StreamReader r = new StreamReader("appsettings.json"))
        {
            ...
            string[] Default = new string[] { email, password };
    ...
    {
        String[] Default = GetDefaultMember();

        if (Default[0]==Email && Default[1]==Password)
        {
            return true;
        }
        else
    ...
    public IEnumerable<Schedule> SearchMember(string name)
    {
        IEnumerable<Schedule> searchResult = null;

        var scheduleSearch = from schedule in Schedules
                           where schedule.Movie.Title.ToLower().Contains(name.ToLower())
                           select schedule;
        searchResult = scheduleSearch;
    ...
    public void UpdateSchedule(Schedule schedule)
    {
        if (schedule != null)
        {
            Schedule scheduleToUpdate = Schedules.FirstOrDefault(s => s.Id == schedule.Id);

            Entry(scheduleToUpdate).CurrentValues.SetValues(schedule);
            SaveChanges();
        }
        else
        {
    ...
    {
        return Schedules.SingleOrDefault(mb => mb.Id == ScheduleId);
    }
